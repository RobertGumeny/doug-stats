...
The Claude provider is the reference implementation for ingesting AI tool logs into a normalized provider contract. It uses `history.jsonl` as the single source for session discovery and scans session JSONL files for token totals, task IDs, and session classification.
